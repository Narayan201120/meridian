  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {isLoading ? <View style={styles.loading}><ActivityIndicator /><Text>Loading...</Text></View> : list.length === 0 ? <Text style={styles.empty}>Nothing scheduled — calendar-aware work will show up here.</Text> : <View style={styles.list}>{list.map((t) => <TaskCardConnected key={t.id} task={t} />)}</View>}
      </ScrollView>
    </SafeAreaView>
  );
}
